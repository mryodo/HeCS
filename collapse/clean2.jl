...
begin
      ...
      plot!( mean(ms34; dims=1)'/binomial(38, 2)/tmp34, mean(κOs34; dims=1)',
            sp =1, lw=2, line = :dash, c = cols[1], label=L"n=38, \mathrm{ original }" 
      )
      plot!( mean(ms34; dims=1)'/binomial(38, 2)/tmp10, mean(κLs34; dims=1)',
            sp=1, lw=3, alpha=0.35, c = cols[1], label=L"n=38, \mathrm{ IBF}"
      )
      plot!( mean(ms34; dims=1)'/binomial(38, 2)/tmp34, mean(κPs34; dims=1)',
            sp =1 , c = cols[1], lw = 3, marker = :square, label=L"n=38, \mathrm{ precon }" 
      ) 
      plot!( mean(ms34; dims=1)'/binomial(38, 2)/tmp34, (maximum(κPs34; dims=1)' .+ minimum(κPs34; dims=1)') ./ 2, sp=1, ribbon = (maximum(κPs34; dims=1)' .- minimum(κPs34; dims=1)') ./ 2, fillalpha = 0.05, c = cols[1], alpha=0., labels="")
      ...
      plot!( mean(ms34; dims=1)'/binomial(38, 2)/tmp34, mean(itOs34; dims=1)',
            sp =2, lw=2, line = :dash, c = cols[1], label=L"n=38, \mathrm{ original }" 
      )
      plot!( mean(ms34; dims=1)'/binomial(38, 2)/tmp34, mean(itLs34; dims=1)',
            sp=2, lw=3, alpha=0.35, c = cols[1], label=L"n=34, \mathrm{ IBF}"
      )
      plot!( mean(ms34; dims=1)'/binomial(38, 2)/tmp34, mean(itPs34; dims=1)',
            sp =2 , c = cols[1], lw = 3, marker = :square, label=L"n=38, \mathrm{ precon }" 
      ) 
      plot!( mean(ms34; dims=1)'/binomial(38, 2)/tmp34, (maximum(itPs34; dims=1)' .+ minimum(itPs34; dims=1)') ./ 2, sp=2, ribbon = (maximum(itPs34; dims=1)' .- minimum(itPs34; dims=1)') ./ 2, fillalpha = 0.05, c = cols[1], alpha=0., labels="")
...
savefig("enriched_triangulation_closer2.pdf")
