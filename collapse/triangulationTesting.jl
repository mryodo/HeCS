...
add = 2 # how many edges do we need to add
...
fl, Σ1, Τ1, edge2Trig, trig2Edg, Ls, Free = greedyCollapseShort( edge2Trig, trig2Edge, perm, edges2 )
...
U, S, V = svd(B2*W)
S = diagm(S)
trim = sum( abs.(diag(S)) .> 1e-6 )
S = S[1:trim, 1:trim]
U = U[:, 1:trim]
V = V[:, 1:trim]

sqrt(condPlus(pinv(C) * P1 * Lu * P1' * pinv(C') ))
condPlus( V' * Π )
...
function repeatTries(N, add, rep)
      ...
      return κ_original, κ_precon, it_original, it_precon, m_sizes, Δ_sizes
end
...
function addCycle(N, rep)
      ...
      while true
            add = add + 2 
            κ_original, κ_precon, it_original, it_precon, m_sizes, Δ_sizes = repeatTries(N, add, rep);
            if (sum(κ_original .== 0))>0
                  break
            end 
            κOs = [ κOs κ_original ] 
            κPs = [ κPs κ_precon ] 
      ...
      @printf "added: %n \n" add
      return κOs, κPs, itOs, itPs, ms, Δs
end


κOs, κPs, itOs, itPs, ms, Δs = addCycle(10, 10)
